
<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?color=FFFFFF&center=true&vCenter=true&lines=Hi+there,+I'm+repzitd+👋;Learning+to+code+day+by+day;Backend+%7C+Mobile+Development" alt="Typing SVG" />
</p>

---

<h1 align="center"> Привет, я repzitd</h1>

<p align="center">
  <em>Учусь программированию второй год и шаг за шагом строю свой путь в IT.</em>
</p>

---

### 🖤 О себе
> Постоянно учусь, пробую новое и стремлюсь расширять горизонты.
- 📌 Специализация: Backend + первые шаги в мобильной разработке.  
- 🎓 Прохожу интенсив в **Школе 21**.  
- 🛠 Языки: **C**, **Python**, **Swift**.  
- 🚀 Коммерческого опыта пока нет — всё впереди.  

---

### 📂 Интересы и развитие
- Backend-системы, архитектура и оптимизация.
- Мобильные приложения (iOS).
- Современные технологии и инструменты разработки.

---

### ⚙️ Мой стек
<p align="center">
  <img src="https://img.shields.io/badge/C-000000?style=for-the-badge&logo=c&logoColor=white" />
  <img src="https://img.shields.io/badge/Python-000000?style=for-the-badge&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/Swift-000000?style=for-the-badge&logo=swift&logoColor=white" />
</p>

---

### 📬 Контакты
- 📧 Email: **ildar@kuzyashev.ru**

---

### 📊 Статистика
<p align="center">
  <img src="https://streak-stats.demolab.com?user=repzitd&theme=dark&hide_border=true&background=000000" alt="GitHub Streak" />
</p>

---

### 📂 Мои проекты
*(скоро здесь появятся ссылки на репозитории, которыми я горжусь)*

---

### 🖤 Мой путь в кодинге
<p align="center">
  <img src="https://media.giphy.com/media/3o6gE5aYpJk5v2ytAA/giphy.gif" width="300" />
</p>

---

<p align="center">
  <img src="https://media.giphy.com/media/xT1R9GHOtI3pZjkZc4/giphy.gif" width="200" />
</p>

